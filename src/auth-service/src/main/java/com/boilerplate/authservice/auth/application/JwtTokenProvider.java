...
import com.boilerplate.authservice.auth.exception.InvalidTokenException;
...
public class JwtTokenProvider implements TokenProvider {
    ...
    public JwtTokenProvider(@Value("${jwt.key}") final String secretKey, @Value("${jwt.expiration.access}") final long accessExpirationInMilliSeconds, @Value("${jwt.expiration.refresh}") final long refreshExpirationRefreshInMilliSeconds) {
    ...
    @Override
    public String generateAccessToken(Long payload) {
        return generateAccessToken(payload.toString());
    }

    @Override
    public String generateRefreshToken(Long payload) {
        return generateRefreshToken(payload.toString());
    }
    ...
    private String generateToken(String payload, Long expirationInMilliseconds) {
        ...
        return Jwts.builder().setSubject(payload).setIssuedAt(now).setExpiration(validity).signWith(key, SignatureAlgorithm.HS256).compact();
    }
